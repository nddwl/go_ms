...
type (
	...
	ArticleModel interface {
		...
		UpdateArticleStatus(ctx context.Context, id int64, status int64) error
	}
...
func (m *customArticleModel) UpdateArticleStatus(ctx context.Context, id int64, status int64) error {
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (sql.Result, error) {
		query := fmt.Sprintf("update %s set status = ? where id = ?", m.table)
		return conn.ExecCtx(ctx, query, id, status)
	})
	return err
}
